...
float* KalmanFilterStep_ram2(float z_plus_0, float z_plus_1, kalman_filter_param* kalman_param)
{
...
void KalmanFiltermain_ram2( float* posi, float* velo, kalman_filter_param* kalman_param, int n){
  /* loop the position and velocity array*/
  for(int i = 0; i < n; i++)
  {
    position[i] = *(KalmanFilterStep_ram2(*(posi+i),*(velo+i), kalman_param)+0);
    velocity[i] = *(KalmanFilterStep_ram2(*(posi+i),*(velo+i), kalman_param)+1);
  }
...
void kal_main_ram2()
{
  KalmanFiltermain_ram2( &position[0], &velocity[0], &kalman_parameters, n);
}
